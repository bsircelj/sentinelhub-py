...
class DataRequest(ABC):
    ...
    def _execute_data_download(self, data_filter, redownload, max_threads, raise_download_errors):
        ...
        # TODO: different child classes should have different download client
        if len(filtered_download_list) == 1:
            data_list = [client.download(filtered_download_list[0])]
        else:
            data_list = client.download_data(filtered_download_list, max_threads=max_threads)
...
class OgcRequest(DataRequest):
    ...
    def __init__(self, layer, bbox, *, time='latest', service_type=None, data_source=DataSource.SENTINEL2_L1C,
        ...
        super().__init__(DownloadClient, **kwargs)  # TODO: this is temporal until rate limiting is improved
